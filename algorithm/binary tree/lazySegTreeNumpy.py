...
class lasySegTree:
    def __init__(
        ...
        self.se = np.repeat((0, 0), self.L - 1)
        self.se = np.vstack(
            (self.se.reshape(-1, 2), np.array([(i, i + 1) for i in range(self.L)]))
        )

        for i in range(self.L - 2, -1, -1):
            self.binaryTree[i] = self.funcvv(
                self.binaryTree[2 * i + 1], self.binaryTree[2 * i + 2]
            )
    ...
    def set(self, index, value):
        ...
        while True:
            index = (index - 1) // 2
            self.binaryTree[index] = self.func(
                self.binaryTree[2 * index + 1], self.binaryTree[2 * index + 2]
            )
            if index == 0:
                break
    ...
    def rec_section_set(self, index, start, end, action):
        ...
        # 部分的に一致する場合
            ...
            self.binaryTree[index] = self.funcvv(
                self.binaryTree[index * 2 + 1], self.binaryTree[index * 2 + 2]
            )
        # 完全に一致しない場合
        else:
            return
    ...
    def prop(self, index):
        if not self.lazy[index] == self.identityA:
            self.binaryTree[index] = self.funcva(
                self.binaryTree[index], self.lazy[index]
            )
            if index <= self.L - 2:
                self.lazy[index * 2 + 1] = self.funcaa(
                    self.lazy[index * 2 + 1], self.lazy[index]
                )
                self.lazy[index * 2 + 2] = self.funcaa(
    ...
    def recquery(self, index, start, end):
        ...
        # 部分的に一致する場合
            ...
            return self.funcvv(
                self.recquery(index * 2 + 1, start, end),
                self.recquery(index * 2 + 2, start, end),
            )
        # 完全に一致しない場合
        else:
